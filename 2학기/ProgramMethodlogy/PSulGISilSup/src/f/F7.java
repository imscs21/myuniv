...
public class F7 extends BaseMain {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		int number[][] = new int[5][3]; int newnum;
		int floor_total[] = new int[5]; int line_total[] = new int[3];
		int total=0;
		int ho;
		int i, j;
		...
		// 아파트 호를 나타내는 변수
		// 반복문 사용을 위한 변수
		for(i=0;i<number.length;i++){//floor
			for(j=0;j<number[i].length;j++){//ho
		...
		for(i=0;i<floor_total.length;i++){
			printline((i+1)+"층에 사는 거주자는 모두 "+floor_total[i]+"명 입니다.");
		}
		printline("");
		for(i=0;i<line_total.length;i++){
			printline((i+1)+"호 라인에 사는 거주자는 모두 "+line_total[i]+"명 입니다. ");
		}
		printline("");
